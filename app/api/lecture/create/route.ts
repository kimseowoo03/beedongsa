...
export async function POST(request: Request) {
  ...
  const data: Lecture = await request.json();
  ...
  try {
    const firestoreAnnouncementsRes = await fetch(
      `https://firestore.googleapis.com/v1beta1/projects/${process.env.NEXT_PUBLIC_FIREBASE_PROJECT_ID}/databases/(default)/documents/Lectures/`,
      {
        method: "POST",
        headers: {
    ...
    return new Response(
      JSON.stringify({
        ...
        message: data.temporaryStorage ? "임시 저장 완료" : "강의 등록 완료",
      }),
      {
        status: 200,
      }
